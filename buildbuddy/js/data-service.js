...
class DataService {
    ...
    async getOrCreateCart() {
        try {
            ...
            // Get the LATEST cart for this session
            ...
            const carts = cartsResult?.data || cartsResult || [];
            ...
            if (carts && carts.length > 0) {
                ...
                const newCart = insertResult?.data || insertResult || [];

                if (newCart && newCart.length > 0) {
                    this.currentCartId = newCart[0].cart_id;
                } else {
                    // Try to fetch the created cart
                    ...
                    const fetchedCart = fetchResult?.data || fetchResult || [];
                    if (fetchedCart && fetchedCart.length > 0) {
                        this.currentCartId = fetchedCart[0].cart_id;
                    }
                }
            }
